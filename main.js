document.addEventListener('DOMContentLoaded', () => {
    ...
    // 7. Auth Dropdown Toggle
    const avatarTrigger = document.getElementById('avatar-trigger');
    const authDropdown = document.getElementById('auth-dropdown');

    if (avatarTrigger && authDropdown) {
        avatarTrigger.addEventListener('click', (e) => {
            e.stopPropagation();
            authDropdown.classList.toggle('active');
            avatarTrigger.classList.toggle('trigger-active');
        });

        document.addEventListener('click', (e) => {
            if (!authDropdown.contains(e.target) && !avatarTrigger.contains(e.target)) {
                authDropdown.classList.remove('active');
                avatarTrigger.classList.remove('trigger-active');
            }
        });
